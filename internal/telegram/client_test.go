...
func TestGetUpdatesAsksForWhatItNeedsAndDecodes(t *testing.T) {
	var gotPath string
	var gotForm url.Values
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		gotForm = r.PostForm
		w.Write([]byte(`{"ok":true,"result":[
			{"update_id":11,"message":{"chat":{"id":-100123},"text":"/status"}},
			{"update_id":12,"edited_message":{"chat":{"id":-100123},"text":"ignored"}}
		]}`))
	}))
	defer server.Close()
	...
	if want := "/bot" + testToken + "/getUpdates"; gotPath != want {
		t.Errorf("path = %q, want %q", gotPath, want)
	}
	if got := gotForm.Get("offset"); got != "11" {
		t.Errorf("offset = %q, want the acknowledgement of 10", got)
	}
	if got := gotForm.Get("timeout"); got != "25" {
		t.Errorf("timeout = %q, want the poll window in whole seconds", got)
	}
	if got := gotForm.Get("allowed_updates"); got != `["message"]` {
		t.Errorf("allowed_updates = %q, want messages only", got)
	}

	if len(updates) != 2 {
		t.Fatalf("decoded %d updates, want 2", len(updates))
	}
	if updates[0].UpdateID != 11 || updates[0].Message == nil ||
		updates[0].Message.Text != "/status" || updates[0].Message.Chat.ID != -100123 {
		t.Errorf("first update = %+v", updates[0])
	}
	// An update carrying something other than a message decodes with a nil
	// Message, which the caller has to survive rather than dereference.
	if updates[1].Message != nil {
		t.Errorf("edited message was decoded as a message: %+v", updates[1])
	}
}

func TestGetUpdatesReportsApiErrors(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
	...
	if err == nil {
		t.Fatal("a failed getUpdates was reported as success")
	}
	if !strings.Contains(err.Error(), "409") || !strings.Contains(err.Error(), "Conflict") {
		t.Errorf("error = %v, want the api code and description", err)
	}
}

// A result that is not a list of updates has to be an error, not an empty
// poll: silently returning nothing would look like "no commands" for ever.
func TestGetUpdatesRejectsAnUnexpectedResult(t *testing.T) {
	...
	if _, err := New(testToken, server.URL).GetUpdates(context.Background(), 0, time.Second); err == nil {
		t.Fatal("an object where a list belongs was accepted")
	}
}

func TestGetUpdatesHonoursTheContext(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		<-r.Context().Done()
	}))
	defer server.Close()
	...
	ctx, cancel := context.WithCancel(context.Background())
	cancel()
	if _, err := New(testToken, server.URL).GetUpdates(ctx, 0, time.Second); err == nil {
		t.Fatal("succeeded with a cancelled context")
	}
}

...
Or ask in the chat, which is where you already are when the thought occurs.
...
The command channel is an outbound long poll, so it opens no port and nothing
about the inbound surface changes.

**One process per bot token may poll for commands.** The Bot API hands each
update to a single `getUpdates` caller: start a second one on the same token
and both ends get `409 Conflict` and lose commands at random. That is why the
flag exists and why it is off by default — this daemon cannot see whether
something else already owns the bot. Share a chat with another bot as much as
you like; do not share a token.

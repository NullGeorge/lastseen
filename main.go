...
func run(args []string) error {
	...
	// Off by default, and it has to stay that way. The Bot API hands updates
	// to one getUpdates caller per token: a second one makes both ends trade
	// 409 Conflict and lose commands at random. This daemon cannot tell
	// whether something else already owns the bot, so it never assumes it.
	commands := fs.Bool("commands", false, "answer /status in the chat; only one process per bot token may poll for commands")
	if err := fs.Parse(args); err != nil {
		return err
	}
	...
	if *commands {
		go commandLoop(ctx, api, watches, chat, logger)
	}

	log.Printf("lastseen %s", version.String())
	log.Printf("listening on %s, alerting after %s of silence from: %s, commands=%t",
		cfg.Listen, time.Duration(cfg.Grace), strings.Join(cfg.Monitors, ", "), *commands)
